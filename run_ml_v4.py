...
from detect_peak_v4 import implement_inn, simulate_spectrum
import argparse
import os
...
def neural_model_multitask(X_train, y_train_cls, y_train_pos, X_test, y_test_cls, y_test_pos, lambda_max,lambda_min):
    ...
    ax.set_title('low S/N ~ 1',fontsize=12)
    ...
    plt.xlim(-100, 100)
    plt.ylim(0, 9000)
    plt.title('Low S/N ~ 1', fontsize=16)
    plt.show()
...
def batch_process(pixels_batch, clone, params_file, model, l, plot_spectrum, fit_plot):
    ...
    for i, j in pixels_batch:
        try:
            spec = np.average(clone[:, j-1:j+1, i-1:i+1], axis=(1,2))        # average over 4x4 spatial pixels to improve S/N  
            pred, popt, _, score = implement_inn(params_file, spec, model, plot_spectrum, fit_plot)
            ...
            if pred == 1:
                ...
                if v < 9000 and v > 0:
                    results.append((i, j, v, np.nan,sb,np.nan,vw, np.nan, np.nan, popt[3], score))
                elif v <0 and v > -9000:
                    results.append((i, j, np.nan, v,np.nan,sb,np.nan,vw, np.nan, popt[3], score))
                else:
                ...
                if is_v1_valid and is_v2_valid:
                    ...
                    results.append((i, j, v1, v2, sb1,sb2,vw1,vw2, (popt[3]+popt[6])/2, np.nan, score))

                elif is_swapped:
                    # v1/v2 have the right sign but in opposite roles → swap them
                    results.append((i, j, v2, v1,sb2,sb1,vw2,vw1, (popt[3]+popt[6])/2, np.nan, score))

                elif is_v2_valid:
                    # only v2 is trustworthy
                    results.append((i, j, np.nan, v2,np.nan,sb2,np.nan,vw2, np.nan, popt[6], score))

                elif is_v1_valid:
                    # only v1 is trustworthy
                    results.append((i, j, v1, np.nan,sb1,np.nan,vw1, np.nan, np.nan, popt[3], score))
                ...
                results.append((i, j, np.nan, np.nan, np.nan,np.nan,np.nan,np.nan, np.nan, np.nan, np.nan))
...
def fit_curve(fits_file, params_file, x_start, x_final, y_start, y_final, plot_all=False,load_file=False):
    ...
    if load_file:
        ...
        score_array= np.full((rows, cols), np.nan, dtype=float)
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing Pixels 🚀",dynamic_ncols=True, leave=True):
            ...
            for i, j, blue_val, red_val, blue_sb, red_sb, blue_vw, red_vw, c_mu, s_mu, score in batch_results:
                ...
                central_mu[j, i]=c_mu
                single_mu[j, i]=s_mu
                score_array[j, i]=score
    return blueshift, redshift,blue_brightness, red_brightness, blue_width, red_width, central_mu, single_mu, score_array
...
def main():
    ...
    blueshift, redshift, sb_b, sb_r, vw_b, vw_r, c_mu, s_mu, score_array = fit_curve(
        args.fits_file,
        args.params_file,
        args.x_start,
        args.x_end,
    ...
    #np.save("outputs/blueshift03.npy", blueshift)
    #np.save("outputs/redshift03.npy", redshift)
    #np.save("outputs/blueshift03_sb.npy", sb_b)
    #np.save("outputs/redshift03_sb.npy", sb_r)
    #np.save("outputs/blueshift03_width.npy", vw_b)
    #np.save("outputs/redshift03_width.npy", vw_r)
    #np.save("outputs/central03_mu.npy", c_mu)
    #np.save("outputs/single03_mu.npy", s_mu)
    #np.save("outputs/score_array03.npy", score_array)
