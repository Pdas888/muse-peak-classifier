...
from detect_peak_v3 import implement_inn, simulate_spectrum
import argparse
import os
...
def neural_model_multitask(X_train, y_train_cls, y_train_pos, X_test, y_test_cls, y_test_pos, lambda_max,lambda_min):
    ...
    1) Classification:
       Predict number of emission peaks in a spectrum:
           ...
           1 → single peak
           2 → double peak
           3 → single narrow peak + flat continuum
    ...
    out_class = Dense(4, activation='softmax', name='classification')(x)
    ...
    cm = confusion_matrix(y_test_cls, y_pred_class, labels=[0,1,2,3])
    cm_norm = cm.astype(float) / cm.sum(axis=1, keepdims=True)
    fig, ax = plt.subplots(figsize=(6,5))

    im = ax.imshow(cm_norm, vmin=0, vmax=1, interpolation='nearest', cmap='viridis')

    ax.set_xticks([0,1,2,3])
    ax.set_yticks([0,1,2,3])
    ax.set_xticklabels(['No peak', '1 peak', '2 peaks', 'Narrow peak'],fontsize=12)
    ax.set_yticklabels(['No peak', '1 peak', '2 peaks', 'Narrow peak'],fontsize=12)

    ax.set_xlabel('Predicted class',fontsize=16)
    ax.set_ylabel('True class',fontsize=16)
    ax.set_title('S/N ~ 10',fontsize=12)
    ...
    for i in range(4):
        for j in range(4):
            ax.text(j, i, f"{cm_norm[i,j]:.2f}",
                    ha="center", va="center",
                    color="black" if cm_norm[i,j] > 0.5 else "cyan", fontsize=12)
    ...
    y_test_cls,
    y_pred_class,
    target_names=['No peak', '1 peak', '2 peaks', 'Narrow peak'],
    digits=3
    ))
    ...
    plt.xlabel(r'$\Delta \lambda$ (Å)', fontsize=20)
    plt.ylabel('Count', fontsize=20)
    plt.legend(fontsize=12, loc='upper right')
    plt.xlim(-100, 100)
    plt.ylim(0, 5000)
    plt.title('S/N ~ 10', fontsize=16)
    plt.show()
    ...
    for i, j in pixels_batch:
        try:
            ...
            if pred == 1:
                ...
                if v < 9000 and v > 0:
                    results.append((i, j, v, np.nan,sb,np.nan,vw, np.nan, np.nan, popt[3]))
                    print(f"Added peak at popt[3]={popt[3]:.2f} Å")
                elif v <0 and v > -9000:
                    results.append((i, j, np.nan, v,np.nan,sb,np.nan,vw, np.nan, popt[3]))
                    print(f"Added peak at popt[3]={popt[3]:.2f} Å")
                else:
                ...
                is_v1_valid   = (-950 < v1 <= 6000)
                is_v2_valid   = (-7500 <= v2 < 0)
                is_swapped    = (-7500 <= v1 < 0) and (-950 < v2 <= 6000)

                if is_v1_valid and is_v2_valid:
                    # both in the “correct” ranges
                    results.append((i, j, v1, v2, sb1,sb2,vw1,vw2, (popt[3]+popt[6])/2, np.nan))

                elif is_swapped:
                    # v1/v2 have the right sign but in opposite roles → swap them
                    results.append((i, j, v2, v1,sb2,sb1,vw2,vw1, (popt[3]+popt[6])/2, np.nan))

                elif is_v2_valid:
                    # only v2 is trustworthy
                    print(f"v2 valid but v1 not valid at pixel ({i},{j}): v1={v1:.2f}, v2={v2:.2f}, {popt[3]:.2f}, {popt[6]:.2f}")
                    results.append((i, j, np.nan, v2,np.nan,sb2,np.nan,vw2, np.nan, popt[6]))

                elif is_v1_valid:
                    # only v1 is trustworthy
                    print(f"v1 valid but v2 not valid at pixel ({i},{j}): v1={v1:.2f}, v2={v2:.2f}, {popt[3]:.2f}, {popt[6]:.2f}")
                    results.append((i, j, v1, np.nan,sb1,np.nan,vw1, np.nan, np.nan, popt[3]))
                ...
                results.append((i, j, np.nan, np.nan, np.nan,np.nan,np.nan,np.nan, np.nan, np.nan))
...
def fit_curve(fits_file, params_file, x_start, x_final, y_start, y_final, plot_all=False,load_file=False):
    ...
    if load_file:
        ...
        central_mu=np.load('C:/Users/z5391280/OneDrive - UNSW/Desktop/PhD/MUSE/my_reduction/ML peak detection/Peak classifier/outputs/central05_mu02.npy')
        single_mu=np.load('C:/Users/z5391280/OneDrive - UNSW/Desktop/PhD/MUSE/my_reduction/ML peak detection/Peak classifier/outputs/single05_mu02.npy')
    else:
        ...
        central_mu= np.full((rows, cols), np.nan, dtype=float)
        single_mu= np.full((rows, cols), np.nan, dtype=float)
    ...
    batch_size = 30  # Adjust this based on your system's capabilities
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing Pixels 🚀",dynamic_ncols=True, leave=True):
            ...
            for i, j, blue_val, red_val, blue_sb, red_sb, blue_vw, red_vw, c_mu, s_mu in batch_results:
                ...
                central_mu[j, i]=c_mu
                single_mu[j, i]=s_mu
    return blueshift, redshift,blue_brightness, red_brightness, blue_width, red_width, central_mu, single_mu
...
def main():
    ...
    blueshift, redshift, sb_b, sb_r, vw_b, vw_r, c_mu, s_mu = fit_curve(
        args.fits_file,
        args.params_file,
        args.x_start,
        args.x_end,
    ...
    np.save("outputs/blueshift06.npy", blueshift)
    np.save("outputs/redshift06.npy", redshift)
    np.save("outputs/blueshift06_sb.npy", sb_b)
    np.save("outputs/redshift06_sb.npy", sb_r)
    np.save("outputs/blueshift06_width.npy", vw_b)
    np.save("outputs/redshift06_width.npy", vw_r)
    np.save("outputs/central06_mu02.npy", c_mu)
    np.save("outputs/single06_mu02.npy", s_mu)
