...
def simulate_spectrum(class_label, xmin, xmax, 
    ...
    if class_label == 1:
        ...
        A2 = np.random.choice(np.arange(A2-12, A2+15, 0.1), size=1)[0]
        ...
        mu1 = np.random.choice(np.arange(lmin, l-3, 1), size=1)[0]    # Randomized peak position. However, we employ a separation window of 15 A so that the two peaks may
        mu2 = np.random.choice(np.arange(l+3, lmax, 1), size=1)[0]    # blend but are always distinct - reflecting the real dataset. 
...
def detect_and_fit(spectrum, model, x_vals, lambda_max, lambda_min, s=5315,mu_h1=5225,sigma1=15,mu_h2=5365,sigma2=15):
    ...
    if pred_class == 1:
        ...
        def valid_double(popt, s_ref, sigma_min=16.0):
            try:
        ...
        lower_bounds = [-np.inf, -np.inf, 0.0, x_vals.min(), 15.0, 0.0, x_vals.min(), 15.0]
        ...
        if popt is None:
            ...
            for mu1_try, mu2_try in shifts:
                ...
                p0 = [0.05, 1.0,
                    amp_at(mu1_try), mu1_try, 27.0,
                    amp_at(mu2_try), mu2_try, 30.0]
                try:
